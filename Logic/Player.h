...
inline void UpdateFPSO(FirstPersonController* player, int size)
{
	for(int i = 0; i < size; i++)
	{
		...
		if(GetKey(KeyCode_LeftMouseButton))  {
			...
			player[i].transform->rotation *= Quaternion(0, player[i].pitch, 0, 1);
		...
		Vector3 forward =(player[i].transform->rotation * player[i].camera->rotation) * Vector3(0, 0, 1);
		Vector3 up      = player[i].transform->rotation * Vector3(0, 1, 0);
		Vector3 right 	= player[i].transform->rotation * Vector3(1, 0, 0);
		if(GetKey(KeyCode_W)) {
			player[i].transform->position -= forward * player[i].speed;
		}
		...
		if(GetKey(KeyCode_A)) {
			player[i].transform->position -= right * player[i].speed;
		}
