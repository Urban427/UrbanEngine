...
double intersectX(const Arc& a, const Arc& b, double ly) {
    ...
    if (std::abs(p.x - q.x) < EPS && std::abs(p.y - q.y) < EPS) return p.x;
    if (std::abs(p.y - q.y) < EPS) return (p.x + q.x) * 0.5;
    if (std::abs(p.y - ly) < EPS) return p.x;
    if (std::abs(q.y - ly) < EPS) return q.x;
    ...
    if (std::abs(A) < EPS) {
        if (std::abs(B) < EPS) return (p.x + q.x) * 0.5;
        return -C / B;
    }
...
std::list<Arc>::iterator findArcAbove(std::list<Arc>& beachline, double x, double sweepY ) {
    if (beachline.empty()) return beachline.end();
    auto left = beachline.begin();
    auto right = std::next(left);
    if (right == beachline.end()) return left;
    while (right != beachline.end()) {
        const double breakX = intersectX(*left, *right, sweepY);
        if (x < breakX) return left;
        left = right;
        ++right;
    }
    return left;
...
void handleSiteEvent(std::list<Arc>& beachline, std::priority_queue<Event*, std::vector<Event*>, EventCompare>& events, const Event& site, VoronoiDiagram& diagram) {
    ...
    Arc leftArc{ arcAbove.site, arcAbove.siteIndex, nullptr, arcAbove.leftEdge, nullptr };
    Arc middleArc{ site.point, site.pointIndex, nullptr, nullptr, nullptr };
    Arc rightArc{ arcAbove.site, arcAbove.siteIndex, nullptr, nullptr, arcAbove.rightEdge };
